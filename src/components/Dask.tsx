...
function Dask() {
  ...
  const localColumns: ColumnType[] = JSON.parse(localStorage.getItem("columns") || "[]");
  const [columns, setColumns] = useState<ColumnType[]>(localColumns.length > 0 ? localColumns : defaultColumns);
  ...
  function updateComment(todoId: number, commentIndex: number, newComment: string) {
    setTodos((todos) =>
      todos.map((todo) =>
        todo.id === todoId
          ? {
    ...
    setTodos((todos) =>
      todos.map((todo) =>
        todo.id === todoId
          ? {
              ...todo,
              comments: todo.comments.filter((_, index) => index !== commentIndex),
            }
          : todo
      )
    );
  ...
  function updateTodoTitle(todoId: number, newTitle: string) {
    setTodos(todos.map(todo => (todo.id === todoId ? { ...todo, title: newTitle } : todo)));
  }
